...
const QuizTopicCard = ({ topic, onStartQuiz }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://mcq-battlequest-1.onrender.com/dashboard/${userEmail}`);
  ...
  const handleViewScore = async () => {
    try {
      const response = await axios.get(`https://mcq-battlequest-1.onrender.com/game/${topic._id}`);
